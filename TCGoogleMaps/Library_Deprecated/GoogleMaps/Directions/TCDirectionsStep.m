...
- (id)initWithProperties:(NSDictionary *)properties
{
    self = [super init];
    if (self) {
        _attributedInstructions = [self attributedStringFromHTML:properties[@"html_instructions"]];
        _instructions = [_attributedInstructions string];
        ...
        _startLocation = [TCGoogleMapsUtility coordinateFromDictionary:properties[@"start_location"]];
        _endLocation = [TCGoogleMapsUtility coordinateFromDictionary:properties[@"end_location"]];
        _encodedPath = properties[@"polyline"][@"points"];        
    }
    return self;
}
...
{
    NSMutableAttributedString *mutableAttributedString = [[NSMutableAttributedString alloc] initWithString:html];
    TFHpple *htmlDoc = [TFHpple hppleWithHTMLData:[html dataUsingEncoding:NSUTF8StringEncoding]];
    
    NSArray *boldNodes = [htmlDoc searchWithXPathQuery:@"//b"];
    for (TFHppleElement *element in boldNodes) {
        NSRange range = [[mutableAttributedString string] rangeOfString:element.raw];
        NSAttributedString *attributedString = [[NSAttributedString alloc] initWithString:[element text]
    ...
    TFHppleElement *divElement = [htmlDoc peekAtSearchWithXPathQuery:@"//div"];
    NSRange range = [[mutableAttributedString string] rangeOfString:divElement.raw];
    
    // Insert a new line character '\n' before the <div> tag.
    [[mutableAttributedString mutableString] insertString:@"\n" atIndex:range.location];
    range.location += 1; // +1 because we've inserted '\n'
    
    NSAttributedString *attributedString = [[NSAttributedString alloc] initWithString:[divElement text]
